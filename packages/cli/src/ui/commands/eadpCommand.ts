...
function extractSeiVersions(dependencyOutput: string): Map<string, string> {
  ...
  const pattern1 =
    /([a-zA-Z0-9.\-_]+\.[sS][eE][iI][a-zA-Z0-9.\-_]*):([a-zA-Z0-9.\-_]+):([^\s]+)/g;
...
async function getSeiPackageVersions(
  ...
  try {
    ...
    const settingsGradleExists = await fs
      .access(settingsGradlePath)
      .then(() => true)
      .catch(() => false);
    const settingsGradleKtsExists = await fs
      .access(settingsGradleKtsPath)
      .then(() => true)
      .catch(() => false);
    const buildGradleExists = await fs
      .access(buildGradlePath)
      .then(() => true)
      .catch(() => false);
    const buildGradleKtsExists = await fs
      .access(buildGradleKtsPath)
      .then(() => true)
      .catch(() => false);

    if (
      !settingsGradleExists &&
      !settingsGradleKtsExists &&
      !buildGradleExists &&
      !buildGradleKtsExists
    ) {
      context.ui.addItem(
        {
    ...
    try {
      ...
      if (modules.length > 0) {
        ...
        for (const module of modules) {
          try {
            ...
            console.debug(
              `Could not get dependency insight for module ${module}:`,
              moduleError,
            );
          }
        }
      ...
      const analysisPrompt = generateSeiAnalysisPrompt(
        versions,
        projectRoot,
        'version',
      );
...
function formatVersionComparison(
  ...
  for (const artifact of allArtifacts) {
    ...
    if (beforeVersion && afterVersion) {
      if (beforeVersion !== afterVersion) {
        changes.push(
          `  ${artifact}: ${beforeVersion} -> ${afterVersion} (CHANGED)`,
        );
      } else {
...
function generateSeiAnalysisPrompt(
  versions: Map<string, string>,
  projectRoot: string,
  commandType: 'version' | 'refresh',
  refreshComparison?: string,
): string {
...
async function runDependencyInsightForAllModules(
  projectRoot: string,
  gradleCmd: string,
): Promise<string> {
  const child_process = await import('node:child_process');
...
async function refreshGradlePackages(
  context: CommandContext,
  _args?: string,
): Promise<SlashCommandActionReturn | void> {
  try {
    ...
    try {
      ...
      const beforeOutput = await runDependencyInsightForAllModules(
        projectRoot,
        gradleCmd,
      );
      ...
      const afterOutput = await runDependencyInsightForAllModules(
        projectRoot,
        gradleCmd,
      );
      ...
      try {
        ...
        // Compare versions and show changes
        const comparison = formatVersionComparison(
          beforeVersions,
          afterVersions,
        );
...
export const eadpCommand: SlashCommand = {
  ...
  action: async (
    ...
    if (subCommandName === 'version') {
      return getSeiPackageVersions(
        context,
        args?.substring(subCommandName.length).trim(),
      );
    } else if (subCommandName === 'refresh') {
      return refreshGradlePackages(
        context,
        args?.substring(subCommandName.length).trim(),
      );
    } else {
