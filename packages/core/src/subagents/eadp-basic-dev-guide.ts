...
1. 实体继承关系
   ...
   - 实体如果有以下特性实现相应接口
     ...
     - 需要软删除\`ISoftDelete\`(包含：deleted long类型),默认包含
     - 需要排序功能\`IRank\`(包含：rank)

2. dao层继承关系
   dao层默认继承\`BaseEntityDao<T extends BaseEntity>\` ,在其实现的方法中，删除表示实体实现软删除并包含值，property为实体的一个字段的名称，默认实现了如下方法：
   ...
   - \`PageResult<T> findByPage(Search searchConfig);\` 使用组合条件和分页查询数据集合（含排序）
...
3. service层继承关系
   service默认继承\`BaseEntityService<T extends BaseEntity>\` 需要实现如下方法
   \`\`\`java
   protected BaseEntityDao<T> getDao(){
      return dao;
   }
   \`\`\`
   其中\`dao\`未继承了\`BaseEntityDao\`的dao, service 默认实现了如下方法：
...
4. controller层继承关系
   一般实体Controller默认继承\`BaseEntityController<T extends BaseEntity, D extends BaseEntityDto>\` ,需要实现如下方法
   \`\`\`java
   ...
   其中\`service\`为继承了\`BaseEntityService\`的服务，Controller默认实现了如下方法：
...
5. api层继承关系
   ...
   api层默认实现\`BaseEntityApi<T extends BaseEntityDto>\`,它默认实现了如下接口：
