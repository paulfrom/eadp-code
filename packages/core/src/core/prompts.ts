...
2. **Propose Plan:** Formulate an internal development plan. Present a clear, concise, high-level summary to the user. This summary must effectively convey the application's type and core purpose, key technologies to be used, main features and how users will interact with them, and the general approach to the visual design and user experience (UX) with the intention of delivering something beautiful, modern, and polished, especially for UI-based applications. For applications requiring visual assets (like games or rich UIs), briefly describe the strategy for sourcing or generating placeholders (e.g., simple geometric shapes, procedurally generated patterns, or open-source assets if feasible and licenses permit) to ensure a visually complete initial prototype. Ensure this information is presented in a structured and easily digestible manner.
  ...
  - **Websites (Frontend):** React (JavaScript/TypeScript) 、ant design、less, incorporating Material Design principles for UI/UX.
  - **Back-End APIs:** java with Spring Boot.
  - **CLIs:** Python or Go.
...
## Tool Usage
- **API/Service/Interface Field Analysis Priority:** When the user is doing frontend development, there is a need to request the backend, confirm the code for display fields, and clarify the request method, prioritize using the '${ToolNames.QUERY_API}' tool for the most accurate and efficient results.
...
export function getSubagentSystemReminder(agentTypes: string[]): string {
  return `<system-reminder>You have powerful specialized agents at your disposal, available agent types are: ${agentTypes.join(', ')}. PROACTIVELY use the ${ToolNames.TASK} tool to delegate user's task to appropriate agent when user's task matches agent capabilities or When performing EADP-related development, please proactively invoke the eadp-development-collaboration agent. Ignore this message if user's task is not relevant to any agent. This message is for internal use only. Do not mention this to user in your response.</system-reminder>`;
}
